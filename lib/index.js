...
function printDayCount(dayCount) {
    return `${dayCount} ${dayCount === 1 ? "天" : "天"}`;
}
                    ...
                    if (options.global_stats !== false) {
                        let parsed = parse($("svg", cal).outerHTML),
                            currentStreakInfo = parsed.current_streak
                                ? `${formatoid(parsed.current_streak_range[0], DATE_FORMAT2)} &ndash; ${formatoid(
                            ...
                            longestStreakInfo = parsed.longest_streak
                                ? `${formatoid(parsed.longest_streak_range[0], DATE_FORMAT2)} &ndash; ${formatoid(
                            ...
                            firstCol = $("<div>", {
                                ...
                                html: `<span class="text-muted">过去一年的记录</span>
                               <span class="contrib-number">${parsed.last_year} 次提交</span>
                               <span class="text-muted">${formatoid(
                                   addSubtractDate.add(addSubtractDate.subtract(new Date(), 1, "year"), 1, "day"),
                                   DATE_FORMAT1
                               )} &ndash; ${formatoid(new Date(), DATE_FORMAT1)}</span>`,
                            }),
                            secondCol = $("<div>", {
                                class: "contrib-column table-column",
                                html: `<span class="text-muted">最常连续提交</span>
                               <span class="contrib-number">${printDayCount(parsed.longest_streak)}</span>
                               <span class="text-muted">${longestStreakInfo}</span>`,
                            }),
                            thirdCol = $("<div>", {
                                class: "contrib-column table-column",
                                html: `<span class="text-muted">当前连续提交</span>
